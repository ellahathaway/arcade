...
{
    Task<ProcessResult> RunProcessAsync(string command, string arguments, string workingDirectory = "", bool printOutput = false);
}

public record ProcessResult(string Output, string Error, int ExitCode);

public class ProcessService(TaskLoggingHelper log, int timeout = 10 * 1000) : IProcessService
{
