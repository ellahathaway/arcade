            ...
            internal UnpackedBundle(string localExtractionPath, string identifier, string version, string rootPkgName, bool repacking = false, bool isNested = true)
            {
                ...
                if (!repacking || !isNested)
                {
                    ...
                    if (repacking || isNested)
                    {
                        if (!isNested)
                        {
                            IEnumerable<string> zippedNestedApps = Pkg.GetFiles(LocalExtractionPath, "*.app", SearchOption.AllDirectories);
                            foreach (string appZip in zippedNestedApps)
                            {
                                string tempDest = appZip + ".unzipped";
                                Pkg.ProcessAppBundle(repacking: false, inputPath: appZip, outputPath: tempDest);
                                File.Delete(appZip);
